...
def getBuffSize(tsn = None):
    if tsn and config.has_section('_tivo_' + tsn):
        if config.has_option('_tivo_' + tsn, 'bufsize'):
            try:
                return str(int(strtod(config.get('_tivo_' + tsn, 'bufsize'))/1000)) + 'k'
            except NoOptionError:
                pass
...
def getDuid():
    if config.has_option('Daemon', 'uid'):
        return int(config.get('Daemon', 'uid'))
    else:
        return 1
def getDgid():
    if config.has_option('Daemon', 'gid'):
        return int(config.get('Daemon', 'gid'))
    else:
        return 1
def getDlog():
    if config.has_option('Daemon', 'log'):
        return config.get('Daemon', 'log')
    else:
        return "/dev/null"
def getDpid():
    if config.has_option('Daemon', 'pid'):
        return config.get('Daemon', 'pid')
    else:
        return "/var/run/pyTivo.pid"
